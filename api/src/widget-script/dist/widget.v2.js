(function () {
   ...
   function renderWidget(publicConfig) {
      ...
      formEl.addEventListener('submit', function (submitEvent) {
         ...
         if (response.ok) {
            idempotencyKeyForAttempt = null;
            statusLine.textContent = 'Message sent. Thank you!';
         } else {
            statusLine.textContent = 'Something went wrong. Please try again.';
         }
         }).catch(function () {
         statusLine.textContent = 'Something went wrong. Please check your connection and try again.';
         });
      });
